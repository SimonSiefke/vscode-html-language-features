...
function validateTestCase(testCase, testCaseName) {
  ...
  if (expectedNextCode.join('') !== testCase.nextText) {
    ...
    throw new Error(
      `testcase ${testCaseName} is invalid, nextdom does not match specified edit`
    )
  }
}
...
function gen(fileName) {
  const basicTest = fs.readFileSync(
  ...
  const e2eCases = tests.map(test => {
    ...
    const waitForEdits =
      test.waitForEdits !== 'false' && test.waitForReload !== 'true'
  ...
  fs.writeFileSync(
    path.join(
      __dirname,
      `generated-tests/${fileName.replace(/\.txt$/, '.ts')}`
    ),
    code
  )
}
